...
private const string projectName = "LightInject.Wcf";

private const string portableClassLibraryProjectTypeGuid = "{786C830F-07A1-408B-BD7F-6EE04809D6DB}";
private const string csharpProjectTypeGuid = "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}";

string pathToSourceFile = @"..\src\LightInject.Wcf\LightInject.Wcf.cs";
...
WriteLine("LightInject.Wcf version {0}" , version);
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package/LightInject.Wcf.Source.nuspec");	
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Binary/package/LightInject.Wcf.nuspec");
	PatchNugetVersionInfo(pathToMetadataFile, version);
	NuGet.CreatePackage(pathToMetadataFile, pathToBuildDirectory);
}
...
{
	string pathToMetadata = "../src/LightInject.Wcf/NuGet";
	string pathToPackageDirectory = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package");	
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Wcf.Source.nuspec");	
	string pathToSourceFile = "tmp/" + frameworkMoniker + "/Source/LightInject.Wcf";
	string pathToDestination = Path.Combine(pathToPackageDirectory, "content/" + packageDirectoryName + "/LightInject.Wcf");
	RoboCopy(pathToSourceFile, pathToDestination, "LightInject.Wcf.cs");
	FileUtils.Rename(Path.Combine(pathToDestination, "LightInject.Wcf.cs"), "LightInject.Wcf.cs.pp");
	ReplaceInFile(@"namespace \S*", "namespace $rootnamespace$.LightInject.Wcf", Path.Combine(pathToDestination, "LightInject.Wcf.cs.pp"));
}
...
private void CopyBinaryFile(string frameworkMoniker, string packageDirectoryName)
{
	...
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Wcf.nuspec");
	...
	RoboCopy(pathToBinaryFile, pathToDestination, "LightInject.Wcf.*");
}

private string ResolvePathToBinaryFile(string frameworkMoniker)
{
	var pathToBinaryFile = Directory.GetFiles("tmp/" + frameworkMoniker + "/Binary/LightInject.Wcf/bin/Release","LightInject.Wcf.dll", SearchOption.AllDirectories).First();
...
{
	string pathToProjectFile = Path.Combine(pathToBuildDirectory, @"DNXCORE50/Binary/LightInject.Wcf/project.json");
	DNU.Build(pathToProjectFile, "DNXCORE50");
	
	pathToProjectFile = Path.Combine(pathToBuildDirectory, @"DNX451/Binary/LightInject.Wcf/project.json");
...
{
	string pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Wcf");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Wcf.Tests");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Wcf");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Wcf.Tests");
	NuGet.Restore(pathToProjectDirectory);
...
{
	string pathToTestAssembly = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Wcf.Tests/bin/Release/LightInject.Wcf.Tests.dll");
...
{	
    ...
    MsTest.RunWithCodeCoverage(pathToTestAssembly, pathToPackagesFolder,pathToTestAdapterDirectory, "LightInject.Wcf.dll");
}
...
private void InitializeNugetBuildDirectory(string frameworkMoniker)
{
	...
	{
		var pathToJsonTemplateFile = Path.Combine(pathToBinary, "LightInject.Wcf/project.json_");
		var pathToJsonFile = Path.Combine(pathToBinary, "LightInject.Wcf/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
		pathToJsonTemplateFile = Path.Combine(pathToSource, "LightInject.Wcf/project.json_");
		pathToJsonFile = Path.Combine(pathToSource, "LightInject.Wcf/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
	}				  
}
...
private void Internalize(string frameworkMoniker)
{
	...
	string pathToSourceFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + "/Source/LightInject.Wcf/LightInject.Wcf.cs");
...
{
	string pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Wcf/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
	
	pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Wcf/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
}
...
{	
	var pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Binary\LightInject.Wcf\Properties\AssemblyInfo.cs");	
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);
	pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Source\LightInject.Wcf\Properties\AssemblyInfo.cs");
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);	
...
private void PatchInternalsVisibleToAttribute(string pathToAssemblyInfo)
{
	...
	sb.AppendLine(@"[assembly: InternalsVisibleTo(""LightInject.Wcf.Tests"")]");
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Wcf/LightInject.Wcf.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Wcf/LightInject.Wcf.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
...
private void SetTargetFrameworkProfile()
{
	...
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, @"PCL_111/Binary/LightInject.Wcf/LightInject.Wcf.csproj");
	XDocument xmlDocument = XDocument.Load(pathToProjectFile);	
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Binary\LightInject.Wcf.Tests\LightInject.Wcf.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);	
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Source\LightInject.Wcf.Tests\LightInject.Wcf.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
}